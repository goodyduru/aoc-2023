...
impl Brick {
    fn cmp(&self, other: &Self) -> Ordering {
        self.start
            .2
            .min(self.end.2)
            .cmp(&other.start.2.min(other.end.2))
    }
}
...
fn parse(input: &String) -> Vec<Brick> {
    let mut bricks: Vec<Brick> = input
        ...
        .map(|line| {
            ...
            let mut start = it.0.split(',').map(|c| c.parse::<u16>().unwrap());
            let start = (
                start.next().unwrap(),
                start.next().unwrap(),
                start.next().unwrap(),
            );
            let mut end = it.1.split(',').map(|c| c.parse::<u16>().unwrap());
            let end = (
                end.next().unwrap(),
                end.next().unwrap(),
                end.next().unwrap(),
...
fn fall(
    ...
    supported_by: &mut HashMap<usize, HashSet<i16>>,
    supports: &mut HashMap<usize, HashSet<i16>>,
) {
    let mut max_x = 0;
    let mut max_y = 0;
    ...
    let mut ground: Vec<Vec<(i16, i16)>> = { 0..max_y + 1 }
        .map(|_| { 0..max_x + 1 }.map(|_| (1, -1)).collect())
        .collect();
